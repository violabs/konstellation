...
sealed class DslProp {
    ...
    data class Default(
        ...
        val accessModifier: KModifier = KModifier.PUBLIC,
...
object DslPropRenderer {
    fun toPropertySpec(prop: DslProp): PropertySpec = when (prop) {
        ...
        is DslProp.BooleanProp -> defaultProperty(
            DslProp.Default(prop.name, propTypeBoolean(prop.nullableAssignment), prop.nullableAssignment, prop.defaultValue, KModifier.PROTECTED, prop.kdoc)
        )
        is DslProp.ListProp -> defaultProperty(
            DslProp.Default(prop.name, kpListOf(prop.elementType, nullable = true), prop.nullableAssignment, null, KModifier.PROTECTED, prop.kdoc)
        )
        is DslProp.MapProp -> defaultProperty(
            DslProp.Default(prop.name, kpMapOf(prop.keyType, prop.valueType, nullable = true), prop.nullableAssignment, null, KModifier.PROTECTED, prop.kdoc)
        )
        is DslProp.GroupProp -> defaultProperty(
            DslProp.Default(prop.name, kpListOf(prop.builtClassName, nullable = true), prop.nullableAssignment, null, KModifier.PROTECTED, prop.kdoc)
        )
        is DslProp.MapGroupProp -> defaultProperty(
            DslProp.Default(prop.name, kpMapOf(prop.keyType, prop.valueType, nullable = true), prop.nullableAssignment, null, KModifier.PROTECTED, prop.kdoc)
        )
        is DslProp.BuilderProp -> defaultProperty(
            DslProp.Default(prop.name, prop.originalType, prop.nullableAssignment, null, KModifier.PROTECTED, prop.kdoc)
        )
    }

    fun accessors(prop: DslProp): List<FunSpec> = when (prop) {
        is DslProp.Default -> emptyList()
        is DslProp.BooleanProp -> kotlinPoet {
            functions {
                add {
                    funName = prop.name
        ...
        is DslProp.ListProp -> kotlinPoet {
            functions {
                add {
                    funName = prop.name
                    prop.kdoc?.let { kdoc(it) }
        ...
        is DslProp.GroupProp -> kotlinPoet {
        ...
        is DslProp.MapGroupProp -> kotlinPoet {
        ...
        is DslProp.BuilderProp -> kotlinPoet {
            functions {
                add {
                    funName = prop.name
                    prop.kdoc?.let { kdoc(it) }
